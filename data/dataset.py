...
class Dataset(data.Dataset):
    ...
    def __init__(self, root, data_list_file, phase='train', input_shape=(1, 128, 128), retinex: bool = False):
        self.phase = phase
    ...
    def __getitem__(self, index):
        ...
        if self.retinex is not None:
            R, L = retinex_decomposition(np.array(data))
            if self.retinex == 'R':
                data= Image.fromarray(cv2.cvtColor((R * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY))
            else:
                data = Image.fromarray(L[:, :, 0])
        else:
            data = data.convert('L')
